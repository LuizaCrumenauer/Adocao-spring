...
<div class="wrapper">
    ...
    <div class="container">
        ...
        <div class="form-container" style="max-width: 700px; margin: 0 auto;">
            ...
            <c:if test="${not empty cachorroParaAdotar}">
                <div class="pet-details-info">
                    <h4 class="pet-details-header">Você está prestes a mudar a vida do(a): <strong><c:out value="${cachorroParaAdotar.nome}"/></strong>!</h4>
                    <p><strong>Raça:</strong> <c:out value="${cachorroParaAdotar.raca}"/></p>
                    <p><strong>Sexo:</strong> <c:out value="${cachorroParaAdotar.sexo}"/></p>
                    <p><strong>Porte:</strong> <c:out value="${cachorroParaAdotar.porte}"/></p>
                </div>
            </c:if>

            <c:if test="${empty cachorroParaAdotar && not empty id_cachorro_para_adocao}">
                <div class="message-container info">
                    <p>Informações do cachorro não carregadas, mas o ID <strong>${id_cachorro_para_adocao}</strong> foi recebido. Por favor, tente novamente ou contate o suporte.</p>
                </div>
            </c:if>

            <c:if test="${not empty cachorroParaAdotar}">
                <form action="${pageContext.request.contextPath}/adocao/registrar" method="post" class="form-containerEdicao">
                    <input type="hidden" name="cachorroId" value="${cachorroParaAdotar.id}">

                    <div class="form-group">
                        <label for="informacoes">Registre aqui as informações importantes sobre o (a) <c:out value="${cachorroParaAdotar.nome}"/>:</label>
                        <textarea id="informacoes" name="informacoes" rows="6" required minlength="10"><c:out value="${informacoes_previas}"/></textarea>
                        ...
                        <c:if test="${not empty msg_erro_form_adocao}">
                            <div class="message-container error" style="margin-top: 10px;">
                                <p><c:out value="${msg_erro_form_adocao}"/></p>
                            </div>
                        </c:if>
                    </div>
            ...
            <c:if test="${empty cachorroParaAdotar && empty id_cachorro_para_adocao}">
